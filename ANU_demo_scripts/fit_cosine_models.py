...
for act_idx in range(len(flat_dm_cmd)):
    ...
    if dm_pupil_filt[act_idx]:
        ...
        mask[i-Sw_x-1: i+Sw_x, j-Sw_y-1:j+Sw_y] = 1/((2*Sw_x-1)*(2*Sw_y-1)) # keep centered, normalize by #pixels in window 
...
if debug:
    ...
    plt.xlabel('DM cmd');plt.ylabel('mean windowed intensity')
    plt.show()
