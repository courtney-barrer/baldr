...
from functions import data_structure_functions as struc

# TO DO ... I need to put function in Baldr to initialize each base configuration file (with empty entries). 
# Then another function to combine them to a mode! 
...
tel_config_dict = struc.init_telescope_config_dict(use_default_values = True)
phasemask_config_dict = struc.init_phasemask_config_dict(use_default_values = True)
DM_config_dict = struc.init_DM_config_dict(use_default_values = True)
detector_config_dict = struc.init_detector_config_dict(use_default_values = True)
...
# HOW AND WHERE SHOULD WE DEFINE WVL GRID!?!?!?! - MAYBE IN tel
...
with col5:
    ...
    calsource_pup_geometry = st.selectbox('Pupil geometry',('disk', 'AT', 'UT'), index=0, key='OL_calgeompupil_input')
    
    calibration_source_config_dict['type'] = 'calibration_source'
    calibration_source_config_dict['spectral_type'] = cal_source_type 
    calibration_source_config_dict['temperature'] = cal_source_temp
    calibration_source_config_dict['calsource_pup_geometry'] = calsource_pup_geometry
    calibration_source_config_dict['calsource_power'] =  calsource_power # Watts
...
# create our mode configuration file 
#mode_config_dict = baldr.create_mode_config_dict( tel_config_file, phasemask_config_file, DM_config_file, detector_config_file)



#calibrate_IM = st.checkbox('calibration source type',['blackbody','flat'], index=0, key='OL_calibrationtype')

#if calibrate_IM :
    
    #dx, pup, dm, FPM, det = baldr.create_objects_from_modal_config_dict( mode_config_dict )

    # define wvls, normalize to 1. (baldr.blackbody) , define ph/m2/s/nm as is used in field flux field object...
    
    calibration_field = field 


config_dict_list = [tel_config_dict, phasemask_config_dict, DM_config_dict, detector_config_dict,calibration_source_config_dict]
config_dict_namelist = ['telescope', 'phasemask', 'DM', 'detector', 'calibration_source'] 

# mode configuration files 
json_string = json.dumps(mode_dict)
st.json(json_string, expanded=True)
st.download_button(
        label=f"Download mode configuration file",
        file_name= 'mode_X_configuration_file',
